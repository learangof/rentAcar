    ...
    $.fn.Tabledit = function(options) {
                    ...
                    if (settings.editButton || settings.deleteButton) {
                        ...
                        if ($table.find('th.tabledit-toolbar-column').length === 0) {
                            ...
                            if(settings.addButton){
                                addButton = '<button type="button" id="id_add_record" class="tabledit-accept-button ' + settings.buttons.add.class + '" style="float: none;">' + settings.buttons.add.html + '</button>';                                
                            }
                            $table.find('tr:first').append('<th class="tabledit-toolbar-column">'+addButton +'</th>');
                        }

                        // Create accept button.
                        if (settings.acceptButton) {
                            acceptButton = '<button type="button" class="tabledit-accept-button ' + settings.buttons.accept.class + '" style="float: none;">' + settings.buttons.accept.html + '</button>';
                        }
                        // Create reject button.
                        if (settings.rejectButton) {
                            rejectButton = '<button type="button" class="tabledit-reject-button ' + settings.buttons.reject.class + '" style="float: none;">' + settings.buttons.reject.html + '</button>';
                        }
                        ...
                        var toolbar = '<div class="tabledit-toolbar ' + settings.toolbarClass + '" style="text-align: left;">\n\
                                           <div class="' + settings.groupClass + '" style="float: none;">' + editButton + deleteButton + acceptButton + rejectButton +'</div>\n\
                                           ' + saveButton + '\n\
                                           ' + confirmButton + '\n\
                                           ' + restoreButton + '\n\
                                       </div></div>';
        ...
        var Edit = {
            ...
            submit: function(td) {
                ...
                /**
         * Available actions for accept function, with button as parameter.
         *
         * @type object
         */
        var Accept = {
            submit: function(td) {
                ...
                // Send AJAX request to server.
                var ajaxResult = ajax(settings.buttons.accept.action);
        ...
        var Reject = {
            submit: function(td) {
                ...
                $(td).parent('tr').find('input.tabledit-identifier').attr('disabled', false);
                // Send AJAX request to server.
                var ajaxResult = ajax(settings.buttons.reject.action);
                // Disable identifier hidden input.
                $(td).parents('tr').find('input.tabledit-identifier').attr('disabled', true);
        ...
        if (settings.acceptButton) {
            $table.on('click', 'button.tabledit-accept-button', function(event) {
        ...
        $table.on('click', 'button#id_add_record', function(event) {
            $("#addModal").modal('show');
        });
        if (settings.rejectButton) {
            $table.on('click', 'button.tabledit-reject-button', function(event) {
                if (event.handled !== true) {
